...
namespace Perpetuum.Units.DockingBases
{
    public class UnitHelper
    {
        ...
        {
            T unit = GetUnit<T>(unitEid);
            return unit == null ? throw new PerpetuumException(ErrorCodes.ItemNotFound) : unit;
        }
        ...
        public T GetUnit<T>(long unitEid) where T : Unit
        {
            ...
            T unit = _zoneManager.GetUnit<T>(unitEid);
            if (unit == null)
            {
    ...
    public class DockingBaseHelper
    {
        ...
        {
            EntityDefault market = _entityServices.Defaults.GetByName(DefinitionNames.PUBLIC_MARKET);
        ...
        {
            EntityDefault itemShop = _entityServices.Defaults.GetByName(DefinitionNames.BASE_ITEM_SHOP);
        ...
        {
            EntityDefault storage = _entityServices.Defaults.GetByName(DefinitionNames.PUBLIC_CORPORATE_HANGARS_STORAGE);
        ...
        {
            long[] childrenEid = _entityServices.Repository.GetFirstLevelChildrenByCategoryflags(dockingBase.Eid, CategoryFlags.cf_production_facilities);

            foreach (long facilityEid in childrenEid)
            {
